...
const signin = (data: SignInRequest): Promise<AxiosResponse<any>> => 
  post(URL_LOGIN + URL_SIGN_IN, data);

const signout = (): Promise<AxiosResponse<any>> => 
  post(URL_LOGIN + URL_SIGN_OUT);

const signup = (data: SignUpRequest): Promise<AxiosResponse<any>> => 
  post(URL_LOGIN + URL_SIGN_UP, data);

const updateUser = (data: UpdateUserRequest): Promise<AxiosResponse<any>> => 
  patch(URL_USER + URL_CHANGE_USERINFO, data);

const deleteUser = (data: { password: string }): Promise<AxiosResponse<any>> => 
  del(URL_USER + URL_DELETE_ACCOUNT, data);
