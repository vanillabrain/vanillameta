...
const Analytics = () => {
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      const [summaryRes, eventsRes, metricsRes, funnelRes] = await Promise.all([
        ...
        analyticsService.getFunnelAnalysis(['user_login', 'dashboard_viewed', 'widget_created'], { timeRange }),
      ]);
  ...
  return (
    <Box sx={{ pb: 4 }}>
      ...
      <PageTitleBox title="사용자 행동 분석">
        <Box sx={{ px: 3, mt: 3 }}>
          {/* 시간 범위 선택 */}
          <Box sx={{ mb: 3, display: 'flex', justifyContent: 'flex-end' }}>
            <FormControl sx={{ minWidth: 200 }}>
              <InputLabel>기간</InputLabel>
              <Select value={timeRange} onChange={handleTimeRangeChange} label="기간">
          ...
          {/* 요약 카드 */}
          {analyticsData.summary && <AnalyticsSummaryCards summary={analyticsData.summary} />}
            ...
            <Grid item xs={12} lg={4}>
              <Card>
                <CardContent>
                  ...
                  <Divider sx={{ mb: 2 }} />
                  {analyticsData.summary && <TopEventsTable topEvents={analyticsData.summary.topEvents} />}
                </CardContent>
              </Card>
            </Grid>
            ...
            <Grid item xs={12}>
              <Card>
                <CardContent>
                  ...
                  <Divider sx={{ mb: 2 }} />
                  {analyticsData.funnel && <FunnelChart funnel={analyticsData.funnel.funnel} />}
                </CardContent>
              </Card>
            </Grid>
