...
export const useUserTracking = (options: TrackingOptions = {}) => {
  ...
  // 대시보드 관련 추적 함수들
  const trackDashboardCreated = useCallback((dashboardId: string, templateUsed?: string, widgetCount?: number) => {
  ...
  const trackDashboardEdited = useCallback((dashboardId: string, dashboardName?: string) => {
  ...
  const trackDashboardShared = useCallback((dashboardId: string, shareMethod: string) => {
  ...
  // 위젯 관련 추적 함수들
  const trackWidgetCreated = useCallback((widgetId: string, widgetType: string, chartType?: string) => {
  ...
  const trackWidgetEdited = useCallback((widgetId: string, widgetType: string) => {
  ...
  const trackWidgetInteraction = useCallback((widgetId: string, widgetType: string, interactionType: string) => {
  ...
  // 데이터 관련 추적 함수들
  const trackDatabaseConnected = useCallback((databaseType: string, success: boolean) => {
  ...
  const trackQueryExecuted = useCallback((datasetId: string, queryDuration: number, rowCount: number, success: boolean) => {
    analytics.track(
      success ? EventAction.QUERY_EXECUTED : EventAction.QUERY_FAILED,
      EventCategory.DATA,
      {
        datasetId,
        queryDuration,
  ...
  const trackDataExported = useCallback((format: string, rowCount: number) => {
  ...
  // 사용자 여정 추적 함수들
  const trackUserRegistered = useCallback((registrationMethod: string) => {
    analytics.track(EventAction.USER_REGISTERED, EventCategory.USER, {
      registrationMethod,
    });
  }, [analytics]);
  
  const trackUserLogin = useCallback((loginMethod: string) => {
    analytics.track(EventAction.USER_LOGIN, EventCategory.USER, {
      loginMethod,
    });
  }, [analytics]);
  
  const trackOnboardingProgress = useCallback((step: string, action: 'started' | 'completed' | 'skipped') => {
    const eventAction = 
      action === 'started' ? EventAction.ONBOARDING_STARTED :
      action === 'completed' ? EventAction.ONBOARDING_COMPLETED :
  ...
  // 커스텀 이벤트 추적
  const trackCustomEvent = useCallback((action: string, category: string, data?: any) => {
    analytics.track(action as EventAction, category as EventCategory, data);
  }, [analytics]);
  
  // 차트 렌더링 성능 추적
  const trackChartRenderTime = useCallback((chartType: string, renderTime: number, dataPoints: number) => {
