...
export class DatabaseController {
  ...
  @ApiOperation({
    ...
    description:
      '지원하는 데이터베이스 엔진 타입 목록을 조회합니다 (MySQL, PostgreSQL, Oracle 등).',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      '데이터베이스 연결 정보, 테이블 목록, 데이터셋 목록을 포함한 상세 정보를 조회합니다.',
  })
  @ApiParam({
    name: 'id',
    description: '데이터베이스 ID',
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      '반환할 필드 선택 (쉼표로 구분). 허용된 필드: id, name, description, engine, type, timezone, createdAt, updatedAt, tables, datasets',
  ...
  @ApiOperation({
    ...
    description:
      '데이터베이스 연결을 삭제합니다. 관련된 데이터셋과 위젯이 있으면 삭제할 수 없습니다.',
  })
  @ApiParam({
    name: 'id',
    description: '데이터베이스 ID',
  ...
  @ApiOperation({
    ...
    description:
      '대용량 쿼리 결과를 NDJSON 형식으로 스트리밍합니다. 메모리 효율적인 처리를 위해 청크 단위로 데이터를 전송합니다.',
  })
  @ApiBody({
    type: QueryExecuteDto,
    description: '쿼리 실행 정보',
