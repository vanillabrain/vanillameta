...
export class JobRetryService {
  ...
  async scheduleRetry(job: QueueJob, error?: any): Promise<void> {
    try {
      ...
      this.logger.log(`Job ${job.id} scheduled for retry in ${retryDelay}ms (attempt ${job.retryCount + 1}/${job.maxRetries})`);
   ...
   * 재시도 통계 조회
   */
  async getRetryStatistics(days: number = 7): Promise<{
    totalRetries: number;
    ...
    try {
      ...
      const averageRetryCount = retriedJobs.length > 0 
        ? totalRetries / retriedJobs.length 
        : 0;

      const retrySuccessRate = (successfulRetries + failedRetries) > 0
   ...
   * 벌크 재시도
   */
  async bulkRetry(jobIds: string[], reason?: string): Promise<{
    successful: string[];
    ...
    this.logger.log(`Bulk retry completed: ${successful.length} successful, ${failed.length} failed`);
