...
import { FieldSelection, PredefinedFields, NoFieldSelection } from './common/field-selection/field-selection.decorator';
...
export class TestFieldSelectionController {
  ...
  @FieldSelection({
    allowedFields: [
      ...
      'profile.avatar', 'profile.bio', 'profile.location',
      'settings.theme', 'settings.language',
      'dashboards.id', 'dashboards.title', 'dashboards.description'
    ],
  ...
  @FieldSelection({
    allowedFields: [
      'id', 'name', 'description', 'rowCount', 'createdAt',
  ...
  async getPerformanceTest(@Query('fields') fields?: string, @Query('size') size?: string) {
    ...
    const largeData = {
      ...
      items: Array(itemCount).fill(null).map((_, index) => ({
        id: index + 1,
        name: `Item ${index + 1}`,
    ...
    return {
      fieldSelectionFeature: {
        ...
        examples: {
          ...
          '/test-fields/dashboards?fields=id,title,widgets.id,widgets.name': 'Dashboard with widget summary',
