...
describe('SqlValidationService', () => {
  ...
  describe('SQL Injection Prevention', () => {
    ...
    it('should block INSERT attacks', () => {
      const result = service.validateQuery(
        "SELECT * FROM users WHERE id = 1; INSERT INTO users (name) VALUES ('hacker'); --",
      );
    ...
    it('should block time-based injection', () => {
      const result = service.validateQuery(
        'SELECT * FROM users WHERE id = 1 AND (SELECT SLEEP(10))',
      );
  ...
  describe('Query Structure Validation', () => {
    ...
    it('should handle ORDER BY with auto LIMIT', () => {
      const result = service.validateQuery('SELECT * FROM users ORDER BY name', {
        maxResultLimit: 100,
      });
  ...
  describe('Function Validation', () => {
    it('should allow safe functions', () => {
      const result = service.validateQuery(
        'SELECT COUNT(*), SUM(price), UPPER(name) FROM products LIMIT 100',
      );
  ...
  describe('Risk Level Assessment', () => {
    ...
    it('should assign MEDIUM risk to queries with warnings', () => {
      const result = service.validateQuery(
        'SELECT name FROM users WHERE id = 1 ORDER BY (SELECT COUNT(*) FROM products)',
      );
    ...
    it('should assign HIGH risk to queries with multiple warnings', () => {
      ...
      const query =
        'SELECT USER(), VERSION(), DATABASE(), UNKNOWN_FUNC(), 0x123456 FROM users WHERE id = 1 -- comment /* another comment */';
