...
export class EnhancedConnectionPoolService implements OnModuleDestroy {
  ...
  private readonly performanceHistory = new Map<
    number,
    Array<{ timestamp: Date; queryTime: number; acquireTime: number }>
  >();
  ...
  async createOptimizedPool(
    ...
    try {
      ...
      const optimizedPoolConfig = this.optimizationService.getOptimizedPoolConfig(
        engine,
        isProduction,
      );
  ...
  getPoolStatus(databaseId: number): PoolHealthStatus | null {
    ...
    const failureRate =
      totalOperations > 0 ? metrics.performance.connectionFailures / totalOperations : 0;
   ...
   * 자동 최적화 시도
   */
  private async attemptAutoOptimization(
    databaseId: number,
