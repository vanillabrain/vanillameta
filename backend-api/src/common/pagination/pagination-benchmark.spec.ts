...
describe('Pagination Performance Benchmark', () => {
  ...
  describe('성능 비교: 커서 vs 오프셋', () => {
    ...
    it('첫 페이지 접근 시 성능 비교', async () => {
      ...
      console.log(
        `첫 페이지 - 오프셋: ${offsetDuration.toFixed(2)}ms, 커서: ${cursorDuration.toFixed(2)}ms`,
      );
  ...
  describe('커서 인코딩/디코딩 성능', () => {
    it('대량의 커서 인코딩/디코딩 성능 측정', () => {
      ...
      console.log(
        `${iterations}개 커서 인코딩: ${encodeDuration.toFixed(2)}ms (${(
          encodeDuration / iterations
        ).toFixed(4)}ms/op)`,
      );
      console.log(
        `${iterations}개 커서 디코딩: ${decodeDuration.toFixed(2)}ms (${(
          decodeDuration / iterations
        ).toFixed(4)}ms/op)`,
      );
