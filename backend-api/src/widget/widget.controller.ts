...
export class WidgetController {
  ...
  @ApiOperation({
    summary: '위젯 생성',
    description:
      '새로운 차트 위젯을 생성합니다. 위젯은 차트 타입, 데이터셋, 차트 옵션으로 구성됩니다.',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiOperation({
    summary: '위젯 목록 조회',
    description:
      '생성된 모든 위젯 목록을 조회합니다. 필드 선택 기능을 통해 필요한 필드만 조회할 수 있습니다.',
  })
  @ApiQuery({
    name: 'fields',
    required: false,
    description:
      '반환할 필드 선택 (쉼표로 구분). 허용된 필드: id, title, description, componentId, datasetType, datasetId, option, createdAt, updatedAt',
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      '반환할 필드 선택 (쉼표로 구분). 미리 정의된 widgetWithConfig 필드 셋을 사용합니다.',
  ...
  findOne(@Param('id') id: string, @Query('fields') _fields?: string) {
  ...
  @ApiOperation({
    summary: '위젯 수정',
    description:
      '기존 위젯의 정보를 수정합니다. 제목, 설명, 차트 옵션, 데이터셋 등을 변경할 수 있습니다.',
  })
  @ApiParam({
    name: 'id',
    description: '위젯 ID',
