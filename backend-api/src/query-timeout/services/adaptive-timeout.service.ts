...
export class AdaptiveTimeoutService {
  ...
  constructor(private readonly timeoutConfigService: TimeoutConfigurationService) {
    // 주기적으로 오래된 기록 정리
    setInterval(() => {
  ...
  analyzeTimeoutRequirements(
    ...
    try {
      ...
      // 기본 타임아웃 계산
      const baseTimeout = this.timeoutConfigService.calculateRecommendedTimeout(engine, complexity);
      ...
      if (historicalData && historicalData.length >= this.adaptiveConfig.minExecutionSamples) {
        ...
        recommendedTimeout = Math.round(
          baseTimeout * adaptationMultiplier * this.adaptiveConfig.safetyMultiplier,
        );
