...
테스트가 균형 있고, 목적이 있으며, 개발 목표와 일치하도록 프로젝트의 테스팅 전략에 대한 테스트 구현을 분석합니다.

## 전제 조건 확인

**먼저**, 테스팅 전략 문서가 있는지 확인:
- `.moonklabs/01_PROJECT_DOCS/TESTING_STRATEGY.md` 확인
...
**옵션 A - 전략 문서가 존재하는 경우:**
- `.moonklabs/01_PROJECT_DOCS/TESTING_STRATEGY.md` 로드
- 주요 원칙과 우선순위 추출
...
- 동작 vs 구현 세부사항을 테스트하는 테스트 식별
- 과도한 설정이나 모킹이 있는 테스트 찾기
- 리팩토링 시 자주 깨지는 테스트 위치 파악
...
- 테스트 파일과 함수를 명시
- 수정이 필요한 이유 설명
- 특정 변경사항 제안 (제거, 단순화, 추가)
- 도움이 된다면 개선된 접근법의 예제 제공
...
**중요:** 메트릭이 아닌 정렬과 균형에 집중하세요. 목표는 프로젝트의 실제 요구를 충족하는 테스트입니다.