...
프로젝트 문서 표준을 따라 `.moonklabs/04_GENERAL_TASKS/`에 새로운 일반 태스크를 생성합니다.

## 정확히 다음 10개 항목으로 TODO 생성

1. 태스크 인자 파싱 및 태스크 ID 결정
...
- 인자가 비어있으면 사용자에게 태스크 세부사항 요청
- `.moonklabs/04_GENERAL_TASKS/`의 기존 태스크를 검토하여 순차적 태스크 ID 생성
- 가장 높은 태스크 번호(T###)를 찾아 1 증가
- 형식: `T###_<태스크_제목_스네이크_케이스>.md`
- **중요:** 태스크 ID는 간격 없이 순차적이어야 함
...
- `.moonklabs/03_SPRINTS/*/T*.md`에서 스프린트 태스크 검색
- 완료된 마일스톤에서 유사한 기능 찾기
- **중요:** 중복이 발견되면 중지하고 파일 참조와 함께 사용자에게 보고

### 4. 구현 컨텍스트를 위한 코드베이스 조사

태스크 설명을 기반으로 병렬 서브에이전트를 사용하여:
...
- 기존 코드와의 통합 지점 매핑
- 태스크에 포함할 모든 발견사항 **문서화**

### 5. 표준 템플릿을 사용하여 태스크 파일 생성

`.moonklabs/99_TEMPLATES/task_template.md`의 태스크 템플릿을 기본 구조로 **사용**:

- 템플릿 구조를 정확히 복사
- 파일을 `.moonklabs/04_GENERAL_TASKS/T###_<제목>.md`에 배치
- 타임스탬프 포함: 생성 시간을 위해 `date '+%Y-%m-%d %H:%M:%S'` 실행
- **중요:** 템플릿 구조를 정확히 따르기 - 편차 없음
...
**중요:** 코드 예제를 포함하지 마세요. 구조적 가이드와 파일 참조만 제공하세요.

### 8. 새 태스크로 프로젝트 매니페스트 업데이트

`.moonklabs/00_PROJECT_MANIFEST.md` **업데이트**:

- "## General Tasks" 섹션에 태스크 추가
- 형식: `- [ ] T###: [태스크 제목] - 상태: 시작 전`
- 알파벳/숫자 순서 유지
- 태스크 파일 링크: `[T###](04_GENERAL_TASKS/T###_제목.md)`
- **중요:** 모든 기존 내용 보존
...
🧪 **테스트 접근법**: [따를 테스트 패턴]
⏭️ **다음 단계**: 태스크 세부사항을 검토하고 `/do_task T###`를 실행하여 시작
```