...
- 수행된 작업과 관련된 태스크, 스프린트, 마일스톤을 찾고 결과를 출력
- 현재 스프린트 디렉토리를 찾기 위해 `.moonklabs/03_SPRINTS/`로 이동
- 스프린트 목표와 산출물을 이해하기 위해 스프린트 메타 파일 읽기
- 특정 태스크가 범위에 있다면, 스프린트 디렉토리에서 태스크 파일 찾고 읽기
- 현재 마일스톤에 대한 `.moonklabs/02_REQUIREMENTS/`의 관련 요구사항 식별
- 특히 `.moonklabs/01_PROJECT_DOCS/`와 `.moonklabs/02_REQUIREMENTS/`의 관련 문서 읽기
