...
- <$ARGUMENTS>의 다른 텍스트는 사용자가 제공한 **task ID**, **sprint ID**, 또는 다른 **컨텍스트 식별자**를 나타냅니다
...
1. **먼저 필터링**: 변경사항을 두 그룹으로 분리:
   - **컨텍스트와 관련됨**: 태스크/컨텍스트 구현의 일부인 파일
   - **컨텍스트와 무관함**: 그 외 모든 것
2. **FOCUS**: 첫 번째 커밋을 위해 "컨텍스트와 관련됨" 그룹만 분석
3. **DEFER**: "무관한" 그룹은 잠재적인 나중 커밋을 위해 보관 (사용자가 요청하는 경우에만)
...
- **이유**: 이러한 변경사항이 함께 속하는 이유에 대한 간단한 설명
...
- **중요:** pre-commit 훅을 사용하고 있으며 부족한 점을 보고할 가능성이 있습니다. 이를 수정해야 합니다. 특히 이러한 문제를 다루는 열린 태스크가 있지 않는 한 검증을 건너뛰지 마세요.
- 승인된 메시지로 **커밋 생성**
- 커밋이 성공적으로 생성되었는지 확인
- **중요:** 남은 커밋이 더 있으면 다음 커밋을 위해 3단계로 돌아가기
