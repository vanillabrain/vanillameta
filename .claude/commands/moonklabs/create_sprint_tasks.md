# 스프린트를 위한 태스크 생성 - 탑 다운 실행

통합 구현 가이드와 함께 기존 스프린트를 위한 상세 태스크를 생성합니다.
...
4. 태스크 분해를 위한 스프린트 산출물 분석
5. 구현 가이드가 포함된 개별 태스크 파일 생성
6. 관련 태스크에 ADR 연결
...
- 태스크가 이미 존재하면 사용자에게 재생성 여부나 다른 가이드를 원하는지 질문
...
**중요:** 스프린트 태스크는 범위를 확장하지 않고 문서화된 스프린트 목표와 일치해야 함.
...
- 태스크 구현에 영향을 줄 기술 결정 메모
- 태스크와 하위 태스크의 관계를 구축하고 구현할 내용을 명확히 하기

**범위 충돌** ADR과 스프린트 목표 사이에 충돌이 있으면, 생각해보고 사용자에게 충돌 해결을 요청.

**ADR이 발견되지 않으면:** 진행하되 구현 중에 태스크가 명확히 될 필요가 있을 수 있음을 메모.

### 4. 태스크 분해를 위한 스프린트 산출물 분석

스프린트 목표와 산출물을 기반으로 (병렬 서브에이전트에서 실행):

- 상위 수준 산출물을 구체적이고 구현 가능한 태스크로 분해
- 각 태스크가 일관된 기능이나 컴포넌트를 나타내는지 확인
- 태스크 간의 논리적 종속성 고려
- 기술 가이드를 위해 태스크를 관련 ADR에 매핑
- 하위 태스크가 포함된 태스크가 완전히 생성될 때까지 복잡성 평가 연기

### 5. 구현 가이드가 포함된 개별 태스크 파일 생성

**이제** 식별된 각 태스크에 대해 다음 지침으로 병렬 서브에이전트 실행:
    ...
    - 모든 태스크 파일은 스프린트 디렉토리에 생성되어야 함 (스프린트 메타 파일이 있는 곳)
    - 명명 규칙으로 파일 생성: `T<NN>_S<NN>_<설명적_이름>.md`
    - T01부터 시작하는 순차 번호 사용
    - `.moonklabs/99_TEMPLATES/task_template.md`의 태스크 템플릿 구조를 정확히 따르기
    ...
    **중요:** 코드 예제를 포함하지 마세요. 구조적 가이드와 참조만 제공하세요.
    ...
    - 태스크가 명확한 구현 경로를 가지고 있는지 확인
    - 모든 통합 지점이 문서화되었는지 확인
    - 가이드가 실제 코드베이스 요소를 참조하는지 확인
    - 태스크가 독립적이고 실행 가능한지 확인

    **반복** 모든 태스크에 대해 `### 5. 구현 가이드가 포함된 개별 태스크 파일 생성`
...
복잡성을 위해 생성된 모든 태스크를 검토하고 높은 복잡성 태스크 분할:

**복잡성 평가 프로세스:**

- 설명, 목표, 수락 기준, 하위 태스크를 포함한 각 태스크 파일을 완전히 읽기
- 전체 범위와 도전에 대한 판단을 사용하여 복잡성 평가
- 파일 수나 예상 시간과 같은 단순한 메트릭에 기반하지 않기
- 개념적 난이도, 통합 과제, 미지의 것들 고려
- 태스크 frontmatter에 복잡성을 낮음, 중간, 높음으로 표시

**높은 복잡성으로 표시된 태스크가 있는 경우:**

- 태스크를 낮음 또는 중간 복잡성의 2-3개 작은 태스크로 분할
- 적절한 순차 번호로 새 태스크 파일 생성
- 원래 높은 복잡성 태스크 파일 업데이트 또는 삭제
- 분할된 태스크가 함께 원래 목표를 달성하는지 확인
- 논리적 그룹화와 종속성 유지

**모든 태스크가 최종화된 후:**

- 모든 태스크가 낮음 또는 중간 복잡성만인지 확인
- 태스크 번호가 순차적인지 확인 (T01, T02, T03...)
- 최종 태스크 목록으로 스프린트 메타 파일 업데이트
- 실제 생성된 태스크를 반영하도록 프로젝트 매니페스트 스프린트 섹션 업데이트
- 완료 보고서 생성
...
**출력 형식:**
    ...
    **다음 단계:**
    - 완전성을 위해 태스크 검토
    - 구현을 시작하려면 `/do_task [스프린트의_첫_번째_태스크]` 실행
    ```